...
> 本文保留研究依据与设计候选，不是最终运行契约。已落地的窗口、阈值、恢复优先级、默认综合体能目标、主页与日历交互以 [JingYou Rhythm v1](JINGYOU_RHYTHM.md) 为准；接口字段与缺测行为以 [Backend API](BACKEND_API.md) 为准。最终实现包含独立的近28天积累参考；单次短睡眠可使当天方向偏向恢复。
